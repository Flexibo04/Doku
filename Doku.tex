...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{l l X l}
    ...
    \midrule
    FA01 & Muss & Die Anwendung authentifiziert sich über
                  OAuth2 (\texttt{client\_credentials}) gegenüber der JTL-Cloud-\gls{api}. & Sicherheit \\
    ...
    FA05 & Muss & Die Session-Token-Verifikation erfolgt serverseitig über
                  \gls{jwks}/EdDSA. & Sicherheit \\
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{l l X}
    ...
    NFA04 & Technologie  & Die Anwendung basiert auf dem JTL-Cloud-App-Standard
                           (AppBridge, JTL-\gls{ui}-Bibliothek). \\
...
\begin{longtable}{@{} l l p{9cm} @{}}
  ...
  FA02 & Backend & \texttt{POST /graphql}-Endpunkt: empfängt GraphQL-Abfragen vom
                  ...
                  leitet die Anfrage an die JTL-Cloud-ERP-\gls{api} weiter. \\[4pt]
  ...
  FA07 & Frontend & \texttt{pane-page}: abonniert das \texttt{shell:customer-context-changed}-Event
...
interface DashboardKPIs {
  ...
  orderCount:     number;       // Anzahl Auftraege
...
des verbundenen JTL-Wawi-Mandanten in einem übersichtlichen Dashboard: Umsatz,
Auftragsanzahl, durchschnittlicher Auftragswert, Top-Produkte, aktuelle Aufträge
und Artikel mit niedrigem Lagerbestand.
...
\begin{itemize}[leftmargin=1.5em]
  \item \textbf{Gesamtumsatz} im aktuellen Monat in Euro
  \item \textbf{Anzahl Aufträge} im aktuellen Monat
  \item \textbf{Durchschnittlicher Auftragswert} in Euro
  \item \textbf{Artikel unter Mindestbestand} (Anzahl)
\end{itemize}

\paragraph{Umsatzdiagramm (Mitte)}
Ein Balkendiagramm stellt den täglichen Umsatzverlauf des aktuellen Monats dar.
Die horizontale Achse zeigt das Datum, die vertikale Achse den Tagesumsatz in Euro.

\paragraph{Letzte Aufträge (unten links)}
Eine Tabelle listet die zuletzt eingegangenen Aufträge mit Auftrags-ID,
Datum, Gesamtwert und aktuellem Status. Der Status wird farbig hervorgehoben:
...
\paragraph{Niedrige Lagerbestände (unten rechts)}
Eine Liste zeigt alle Artikel, deren aktueller Lagerbestand den definierten
Mindestbestand unterschreitet, mit Produktname und verbleibendem Bestand.
