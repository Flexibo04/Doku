...
% hier überschreiben damit auch dort Kopfzeile und "X von Y" erscheinen
...
% Bibliografie-Überschrift mit leerer Nummern-Box (einheitliche TOC-Ausrichtung)
\defbibheading{bibnumberline}[\bibname]{%
  \chapter*{#1}%
...
{%
  ...
  \singlespacing
  \renewcommand{\addvspace}[1]{}% Kapitel-Abstände in den Verzeichnissen entfernen
  \listoffigures
  \addcontentsline{toc}{chapter}{\protect\numberline{}Abbildungsverzeichnis}
  \listoftables
  \addcontentsline{toc}{chapter}{\protect\numberline{}Tabellenverzeichnis}
  \lstlistoflistings
  \addcontentsline{toc}{chapter}{\protect\numberline{}Quelltextverzeichnis}
}
...
Die im Projekt eingesetzten Ressourcen - Entwicklungsrechner, Betriebssystem,
Entwicklungsumgebung und verwendete Bibliotheken - sind im Anhang~\ref{anhang:ressourcen}
aufgeführt. Bei der Softwareauswahl wurde durchgehend auf kostenlose Werkzeuge gesetzt,
sodass für das Projekt keine zusätzlichen Lizenzkosten anfielen.

\section{Entwicklungsprozess und Iterationsplanung}

Für das Abschlussprojekt wurde ein iterativ-inkrementeller Ansatz gewählt: Die Umsetzung
wurde in abgeschlossene Teilbereiche (Backend, Frontend, Qualitätssicherung) gegliedert,
die nacheinander, aber mit regelmäßiger Überprüfung der Zwischenergebnisse, durchlaufen
wurden. Auf dieser Grundlage wurde vor Beginn der Implementierung ein Iterationsplan
erstellt, der die Umsetzung in folgende Schritte gliedert:
...
Zur Beurteilung der Wirtschaftlichkeit werden im Folgenden die anfallenden
Projektkosten ermittelt und den zu erwartenden Erträgen gegenübergestellt, woraus
sich die Amortisationsdauer des Projekts ableiten lässt.
...
betrieblichen Kostensätzen der \gls{ris}. Tabelle~\ref{tab:kosten} schlüsselt die
Projektkosten auf.
...
von zehn Monaten (vgl. Abbildung~\ref{fig:breakeven}). Der tatsächliche Lizenzpreis
wird nach Abschluss der Beta-Phase auf Basis von Markt- und Kundenfeedback festgelegt.
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}
    ...
    \end{axis}
  \end{tikzpicture}
  \caption[Break-Even-Analyse: Amortisation nach zehn Monaten]{Break-Even-Analyse: Amortisation nach zehn Monaten\\\footnotesize Quelle: Eigene Darstellung}
  \label{fig:breakeven}
\end{figure}
...
JTL-Cloud-Plattform integriert. Als Architektur wurde eine Monorepo-Struktur gewählt, bei der Frontend und Backend in einem gemeinsamen Quellcode-Repository verwaltet werden:
...
vergleichbare Grundlage bilden.\footcite{nodejs_docs} Das Ergebnis fasst
Tabelle~\ref{tab:nutzwert} zusammen. Die Gewichtung (Gew.) spiegelt die Bedeutung
...
unabhängige Weiterentwicklung beider Schichten. Einen Überblick über die Schichten
gibt Abbildung~\ref{fig:architektur}, das zugehörige Komponentendiagramm
Abbildung~\ref{fig:komponenten}.
...
\begin{figure}[H]
  ...
  \caption[Systemarchitektur, Übersicht]{Systemarchitektur, Übersicht\\\footnotesize Quelle: Eigene Darstellung}
...
\begin{figure}[H]
  ...
  \caption[Komponentendiagramm der Systemarchitektur]{Komponentendiagramm der Systemarchitektur\\\footnotesize Quelle: Eigene Darstellung}
...
Darstellung der \glspl{kpi} geachtet. Die Dashboard-Hauptansicht sieht
Kennzahlkarten, ein Diagramm für den Umsatzverlauf, eine Auflistung der jüngsten
Bestellungen sowie eine Reiterleiste zur Wahl des Auswertungszeitraums (täglich,
wöchentlich, monatlich, jährlich oder benutzerdefiniert) vor. Mockups befinden sich
im Anhang~\ref{anhang:mockups} (Abbildung~\ref{fig:mockup}).
...
Authentifizierung, Token-Verifikation und Weiterleitung; Tabelle~\ref{tab:api_endpunkte}
listet die Endpunkte auf.
...
\texttt{client\_credentials}-Flow.\footcite{jtl_auth,oauth2_rfc}
...
\begin{figure}[H]
  ...
  \caption[Sequenzdiagramm: Authentifizierungs- und Token-Verifikationsablauf]{Sequenzdiagramm: Authentifizierungs- und Token-Verifikationsablauf\\\footnotesize Quelle: Eigene Darstellung}
  \label{fig:sequenz}
\end{figure}
...
\texttt{computeDateRange()} berechnet werden. Diese Hilfsfunktion nimmt den im
Dashboard gewählten Zeitraum entgegen - täglich, wöchentlich, monatlich, jährlich
oder einen benutzerdefinierten Zeitraum - und liefert dafür \gls{iso}-Start- und
Endzeitstempel sowie den passenden Gruppierungswert zurück. Voreingestellt ist die
Monatsansicht, die den Umsatz der letzten zwölf Monate monatsweise gruppiert. Die
übrigen drei Queries sind zeitraumunabhängig und liefern stets aktuelle Bestands-
und Bestelldaten. Das Ergebnis wird als gebündeltes \texttt{DashboardData}-Objekt
an die Dashboard-Seite zurückgegeben.
...
Über eine Reiterleiste am oberen Rand lässt sich der Auswertungszeitraum wählen -
täglich, wöchentlich, monatlich, jährlich oder ein benutzerdefinierter Zeitraum;
voreingestellt ist die Monatsansicht der letzten zwölf Monate. Die Darstellung
gliedert sich in mehrere Bereiche: Im oberen Bereich werden vier
\glspl{kpi} - Gesamtumsatz, Anzahl der Bestellungen, Anzahl aktiver Kunden und
durchschnittlicher Bestellwert - in kompakten Kennzahlkarten ausgegeben. Darunter
folgen zwei Diagramme der Bibliothek Recharts: ein Liniendiagramm des Umsatzverlaufs
im gewählten Zeitraum sowie ein Balkendiagramm der fünf umsatzstärksten Artikel.
Den Abschluss bilden zwei Listen: eine Bestellliste mit den jüngsten Aufträgen -
jede Zeile enthält Firmenname, Bestellnummer, Datum, Bruttobetrag sowie einen
farbcodierten Statusbadge, der den aktuellen Auftragsstatus wie \enquote{Offen}
oder \enquote{Versendet} auf einen Blick signalisiert - sowie eine Übersicht der
...
Anhang~\ref{anhang:screenshot_app} (Abbildung~\ref{fig:screenshot_app}).
...
ausgeführt. Für künftige automatisierte Unit-Tests der Hilfsfunktionen wurde zudem das
Test-Framework \textbf{Vitest} eingerichtet; im Rahmen des Prototyps wurde jedoch auf
automatisierte Tests verzichtet.
...
% ============================================================
\cleardoublepage
\printbibliography[heading=bibnumberline, title={Literaturverzeichnis}]
...
Tabelle~\ref{tab:lastenheft_fa} listet die funktionalen Anforderungen auf.
...
Die nicht-funktionalen Anforderungen fasst Tabelle~\ref{tab:lastenheft_nfa} zusammen.
...
\begin{figure}[H]
  ...
  \caption[Mockup der Dashboard-Hauptansicht]{Mockup der Dashboard-Hauptansicht\\\footnotesize Quelle: Eigene Darstellung}
  \label{fig:mockup}
\end{figure}
...
Tabelle~\ref{tab:pflichtenheft_matrix} ordnet jeder Anforderung die konkrete technische Umsetzung zu.
...
const ErpPage: React.FC<IErpPageProps> = ({ appBridge }) => {
  ...
  const load = useCallback(async () => {
    ...
    const result = await fetchDashboard(token, computeDateRange('monthly'));
    setData(result);
  }, [appBridge]);
  ...
  return (
    <div className="flex flex-col gap-4 p-4 bg-gray-50">
      <div className="flex gap-3">
        ...
        <KpiCard label="Aktive Kunden" value={String(data.kpis.totalCustomers)} />
        <KpiCard label=" Avg Bestellwert" value={formatEur(data.kpis.avgOrderValue)} />
      </div>
      <LineChart data={data.revenuePoints.slice(-12)}>
        <Line dataKey="revenue" stroke="#1a56db" />
      </LineChart>
      {data.recentOrders.map(order => (
        <OrderRow key={order.salesOrderNumber} order={order} />
      ))}
      {/* Top-Artikel und niedrige Bestaende analog */}
    </div>
  );
};
\end{lstlisting}
...
\begin{figure}[H]
  ...
  \caption[Screenshot des fertigen ERP-Dashboards]{Screenshot des fertigen ERP-Dashboards\\\footnotesize Quelle: Eigener Screenshot der Anwendung}
...
{
  "query": "{ orders(first: 5) { nodes { salesOrderNumber status } } }",
  "variables": {}
}
\end{lstlisting}
...
/**
 * Ruft alle Dashboard-Daten eines Mandanten parallel ab.
 * Fuehrt fuenf GraphQL-Abfragen gleichzeitig aus (Promise.all).
 *
 * @param sessionToken - Aktueller Session-Token aus der AppBridge
 * @param range - Auswertungszeitraum (Ergebnis von computeDateRange())
 * @returns Promise<DashboardData> mit allen Anzeigedaten
 * @throws Error bei Netzwerkfehler oder ungueltigem Token
 *
 * @example
 * const data = await fetchDashboard(token, computeDateRange('monthly'));
 * console.log(data.kpis.totalRevenue); // z. B. 48320.50
 */
...
/** Gebuendelte Dashboard-Daten eines Mandanten */
interface DashboardData {
  kpis:          DashboardKPIs;    // Kennzahlen-Karten
  revenuePoints: RevenuePoint[];   // Umsatzverlauf (Liniendiagramm)
  topProducts:   TopProduct[];     // Top-Artikel nach Umsatz
  recentOrders:  Order[];          // Letzte Auftraege
  lowStockItems: LowStockItem[];   // Artikel mit niedrigem Bestand
}

/** Kennzahlen fuer die KPI-Karten */
interface DashboardKPIs {
  totalRevenue:   number;   // Gesamtumsatz im Zeitraum
  totalOrders:    number;   // Anzahl Bestellungen im Zeitraum
  totalCustomers: number;   // Anzahl aktiver Kunden im Zeitraum
  avgOrderValue:  number;   // Durchschnittlicher Bestellwert
}

/** Datenpunkt fuer den Umsatzverlauf */
interface RevenuePoint {
  label:   string;  // Perioden-Label (z. B. Monat)
  revenue: number;  // Umsatz in der Periode
  orders:  number;  // Anzahl Bestellungen in der Periode
}
...
interface Order {
  ...
  salesOrderDate:   string;       // ISO-Datum
...
interface LowStockItem {
  ...
  quantityLockedForShipment: number;
}
\end{lstlisting}
...
Anzahl der Bestellungen, durchschnittlicher Auftragswert, den Umsatzverlauf über einen
wählbaren Zeitraum sowie die jüngsten Aufträge.
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.95\textwidth]{image}
  \caption[Dashboard-Ansicht (Screenshot)]{Dashboard-Ansicht (Screenshot)\\\footnotesize Quelle: Eigener Screenshot der Anwendung}
  \label{fig:screenshot_dashboard}
\end{figure}

Das Dashboard (Abbildung~\ref{fig:screenshot_dashboard}) gliedert sich in die
folgenden Bereiche. Über die Reiter am oberen Rand lässt sich der Auswertungszeitraum
umschalten (täglich, wöchentlich, monatlich, jährlich oder benutzerdefiniert);
voreingestellt ist die Monatsansicht der letzten zwölf Monate.

\paragraph{\gls{kpi}-Karten (oben)}
Vier Kennzahlenkarten zeigen auf einen Blick:
\begin{itemize}[leftmargin=1.5em]
  \item \textbf{Gesamtumsatz} im ausgewerteten Zeitraum in Euro
  \item \textbf{Anzahl der Bestellungen} im selben Zeitraum
  \item \textbf{Anzahl aktiver Kunden}
  \item \textbf{Durchschnittlicher Bestellwert} je Auftrag
\end{itemize}

\paragraph{Umsatzverlauf und Top-Artikel (Mitte)}
Ein Liniendiagramm stellt den Umsatzverlauf der letzten zwölf Monate dar; daneben
zeigt ein Balkendiagramm die fünf umsatzstärksten Artikel.

\paragraph{Letzte Aufträge (unten links)}
...
(vgl. Tabelle~\ref{tab:auftragsstatus}):
...
\paragraph{Niedriger Bestand (unten rechts)}
Eine Liste zeigt die Artikel, deren verfügbarer Lagerbestand einen kritischen Wert
unterschreitet, mit Artikelnummer, Bezeichnung sowie verfügbarer und gesamter Menge.
...
Tabelle~\ref{tab:fehlerbehebung} fasst häufige Probleme und ihre Lösungen zusammen.
