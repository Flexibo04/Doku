...
im gleichen Ökosystem (Node.js / TypeScript) betrieben werden und somit eine
...
Express erzielt mit einem Nutzwert von 4{,}82 das beste Ergebnis und wird daher
...
jedem Node.js-Framework funktionieren. Express erhält die volle Punktzahl, da
es der am weitesten verbreitete Standard ist, für den alle Bibliotheken primär
getestet sind; NestJS erhält einen Punkt Abzug, weil die zusätzliche
...
war minimaler Overhead entscheidend. Express erfordert kein
Framework-spezifisches Architekturmuster und erlaubt den sofortigen Einstieg.
NestJS verlangt Kenntnisse in Decorators, Dependency Injection und
Modulstrukturen, was für einen Prototyp unverhältnismäßig aufwändig ist und
daher mit 2 bewertet wird.

\textbf{TypeScript-Unterstützung (Gew. 4):} TypeScript ist im gesamten
Monorepo durchgehend eingesetzt. NestJS und Fastify sind TypeScript-first
entwickelt (Bewertung 5), während Express auf Community-Typings
(\texttt{@types/express}) angewiesen ist (Bewertung 4).

\textbf{Kenntnisstand (Gew. 4):} Express war aus früheren Projekten bereits
bekannt, was die Implementierungszeit direkt verkürzt (Bewertung 5).
NestJS und Fastify waren neu und hätten zusätzliche Einarbeitungszeit
erfordert (Bewertung 2).

\textbf{Prototyp-Eignung (Gew. 4):} Express erlaubt es, Endpunkte ohne
Boilerplate direkt zu definieren. NestJS ist als Enterprise-Framework für einen
Prototyp überdimensioniert und erhält daher die niedrigste Bewertung (2).
Fastify liegt mit seinem schlanken Plugin-System dazwischen (Bewertung 4).
...
in die JTL-Wawi eingebettet wird; ein \texttt{iframe} stellt dabei einen
eingebetteten Browser-Rahmen dar, der eine separate Webanwendung innerhalb der
Wawi-Oberfläche isoliert ausführt.
Der App Shell übernimmt dabei Authentifizierungstoken-Verwaltung und Kommunikation
mit dem Host-System, also der übergeordneten JTL-Wawi-Anwendung, die den Rahmen
...
Die Implementierung der Benutzeroberfläche erfolgte mit React 19 und der
...
manuell gegen die JTL-Cloud-\gls{api} und über das \gls{api}-Testwerkzeug Bruno
ausgeführt; Unit-Tests einzelner Hilfsfunktionen wurden mit Vitest realisiert.
...
\textbf{Software}
\begin{itemize}[leftmargin=1.5em]
  \item Ubuntu 24.04 LTS -- Betriebssystem
  \item Visual Studio Code -- Entwicklungsumgebung
  \item Node.js 23 -- Backend-Laufzeitumgebung
  \item Express 5.1 -- HTTP-Server-Framework
  \item React 19 -- Frontend-Framework
  \item TypeScript 6.x -- Typisierte JavaScript-Erweiterung
  \item Vite 8.x -- Build-Tool und Dev-Server
  \item Turbo 2.5 -- Monorepo-Orchestrierung
  \item Tailwind CSS 4.x -- Utility-first CSS-Framework
  \item Recharts 3.x -- Diagrammbibliothek für React
  \item graphql-request 7.x -- Typsicherer GraphQL-Client
  \item @jtl-software/cloud-apps-core -- JTL AppBridge
  \item @jtl-software/platform-ui-react -- JTL UI-Komponenten
  \item jose 6.0 -- JWT-Verifikation (EdDSA)
  \item Vitest -- Testframework
  \item Git / GitHub -- Versionskontrolle
  \item Bruno / Postman -- \gls{api}-Testing
  \item MiKTeX / TeX Live -- Textsatzsystem \LaTeX
\end{itemize}
